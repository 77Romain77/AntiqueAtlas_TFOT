 ...
 *
 * <p>This edition deliberately registers no network channel, so a client can
 * join servers that do not have Antique Atlas installed.</p>
 */
@Mod(AntiqueAtlas.ID)
public class AntiqueAtlas {
    ...
    public static ResourceLocation id(String... path) {
        return path[0].contains(":")
                ? ResourceLocations.parse(String.join(".", path))
                : ResourceLocations.of(ID, String.join(".", path));
    }
